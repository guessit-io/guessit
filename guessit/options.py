...
def build_argument_parser() -> ArgumentParser:
    ...
    naming_opts.add_argument(
        "--includes", action="append", dest="includes", default=None, help="List of properties to be detected"
    )
    naming_opts.add_argument(
        "--excludes", action="append", dest="excludes", default=None, help="List of properties to be ignored"
    )
    ...
    output_opts.add_argument(
        "-v", "--verbose", action="store_true", dest="verbose", default=None, help="Display debug output"
    )
    output_opts.add_argument(
        "-P",
        "--show-property",
    ...
    output_opts.add_argument(
        "-i",
        "--output-input-string",
        dest="output_input_string",
        action="store_true",
        default=False,
        help="Add input_string property in the output",
    )
    ...
    if filepath.endswith((".yaml", ".yml")):
        try:
            ...
            raise ConfigurationException(
                ...
                f'PyYAML should be installed to support "{filepath}" file'
            ) from err
