...
def source(config: dict[str, Any]) -> Rebulk:
    ...
    rebulk.regex(*build_source_pattern("VHS", suffix=optional(rip_suffix)), value={"source": "VHS", "other": "Rip"})
    rebulk.regex(*build_source_pattern("CAM", suffix=optional(rip_suffix)), value={"source": "Camera", "other": "Rip"})
    rebulk.regex(
        *build_source_pattern("HD-?CAM", suffix=optional(rip_suffix)), value={"source": "HD Camera", "other": "Rip"}
    )
    # For TS, we remove 'streaming_service.suffix' tag to avoid "Shots" being guessed as Showtime and TS.
    rebulk.regex(
        *build_source_pattern("TELESYNC", "TS", suffix=optional(rip_suffix)),
        value={"source": "Telesync", "other": "Rip"},
        tags=["video-codec-prefix"],
        overrides=["tags"],
    )
    rebulk.regex(
        *build_source_pattern("HD-?TELESYNC", "HD-?TS", suffix=optional(rip_suffix)),
        value={"source": "HD Telesync", "other": "Rip"},
    )
    rebulk.regex(*build_source_pattern("WORKPRINT", "WP"), value="Workprint")
    rebulk.regex(
        *build_source_pattern("TELECINE", "TC", suffix=optional(rip_suffix)),
        value={"source": "Telecine", "other": "Rip"},
    )
    rebulk.regex(
        *build_source_pattern("HD-?TELECINE", "HD-?TC", suffix=optional(rip_suffix)),
        value={"source": "HD Telecine", "other": "Rip"},
    )
    rebulk.regex(
        *build_source_pattern("PPV", suffix=optional(rip_suffix)), value={"source": "Pay-per-view", "other": "Rip"}
    )
    rebulk.regex(*build_source_pattern("SD-?TV", suffix=optional(rip_suffix)), value={"source": "TV", "other": "Rip"})
    rebulk.regex(
        *build_source_pattern("TV", suffix=rip_suffix),  # TV is too common to allow matching
        value={"source": "TV", "other": "Rip"},
    )
    rebulk.regex(*build_source_pattern("TV", "SD-?TV", prefix=rip_prefix), value={"source": "TV", "other": "Rip"})
    rebulk.regex(*build_source_pattern("TV-?(?=Dub)"), value="TV")
    rebulk.regex(
        *build_source_pattern("DVB", "PD-?TV", suffix=optional(rip_suffix)),
    ...
    rebulk.regex(
        *build_source_pattern("HD-?TV", suffix=optional(rip_suffix)),
        conflict_solver=demote_other,
        value={"source": "HDTV", "other": "Rip"},
    )
    rebulk.regex(
        *build_source_pattern("TV-?HD", suffix=rip_suffix),
        conflict_solver=demote_other,
        value={"source": "HDTV", "other": "Rip"},
    )
    rebulk.regex(
        *build_source_pattern("TV", suffix="-?(?P<other>Rip-?HD)"),
        conflict_solver=demote_other,
        value={"source": "HDTV", "other": "Rip"},
    )

    rebulk.regex(
        *build_source_pattern("VOD", suffix=optional(rip_suffix)), value={"source": "Video on Demand", "other": "Rip"}
    )

    rebulk.regex(*build_source_pattern("WEB", "WEB-?DL", suffix=rip_suffix), value={"source": "Web", "other": "Rip"})
    # WEBCap is a synonym to WEBRip, mostly used by non english
    rebulk.regex(
        *build_source_pattern("WEB-?(?P<another>Cap)", suffix=optional(rip_suffix)),
        value={"source": "Web", "other": "Rip", "another": "Rip"},
    )
    rebulk.regex(*build_source_pattern("WEB-?DL", "WEB-?U?HD", "DL-?WEB", "DL(?=-?Mux)"), value={"source": "Web"})
    rebulk.regex("(WEB)", value="Web", tags="weak.source")

    rebulk.regex(
        *build_source_pattern("HD-?DVD", suffix=optional(rip_suffix)), value={"source": "HD-DVD", "other": "Rip"}
    )

    rebulk.regex(
        *build_source_pattern("Blu-?ray", "BD", "BD[59]", "BD25", "BD50", suffix=optional(rip_suffix)),
        value={"source": "Blu-ray", "other": "Rip"},
    )
    rebulk.regex(
        *build_source_pattern("(?P<another>BR)-?(?=Scr(?:eener)?)", "(?P<another>BR)-?(?=Mux)"),  # BRRip
    ...
    rebulk.regex(
        *build_source_pattern("UHD-?TV", suffix=optional(rip_suffix)),
        conflict_solver=demote_other,
        value={"source": "Ultra HDTV", "other": "Rip"},
    )
    rebulk.regex(
        *build_source_pattern("UHD", suffix=rip_suffix),
        conflict_solver=demote_other,
        value={"source": "Ultra HDTV", "other": "Rip"},
    )

    rebulk.regex(
        *build_source_pattern("DSR", "DTH", suffix=optional(rip_suffix)), value={"source": "Satellite", "other": "Rip"}
    )
    rebulk.regex(*build_source_pattern("DSR?", "SAT", suffix=rip_suffix), value={"source": "Satellite", "other": "Rip"})
        ...
        return not matches.holes(start, end, predicate=lambda m: m.value.strip(seps)) and not matches.range(
            ...
            predicate=(
                lambda m: (
                    not m.private
                    and (
                        m.name not in ("screen_size", "color_depth")
                        and (m.name != "other" or "uhdbluray-neighbor" not in m.tags)
                    )
                )
            ),
        )
    ...
    def when(self, matches: Matches, context: dict[str, Any] | None) -> Any:
        ...
        for filepart in matches.markers.named("path"):
            for match in matches.range(
                filepart.start,
                filepart.end,
                predicate=(lambda m: not m.private and m.name == "source" and m.value == "Blu-ray"),
            ):
                ...
                if not other or not self.validate_range(matches, other.end, match.start):
                    ...
                    if not other or not self.validate_range(matches, match.end, other.start):
                        if not matches.range(
                            filepart.start,
                            filepart.end,
                            predicate=(lambda m: m.name == "screen_size" and m.value == "2160p"),
                        ):
                            continue
    ...
    def when(self, matches: Matches, context: dict[str, Any] | None) -> Any:
        ...
        for filepart in matches.markers.named("path"):
            for match in matches.range(filepart.start, filepart.end, predicate=lambda m: m.name == "source"):
                ...
                if not seps_before(match) and not matches.range(
                    match.start - 1, match.start - 2, lambda m: "source-prefix" in m.tags
                ):
                    if match.children:
                        ret.extend(match.children)
                    ret.append(match)
                    continue
                if not seps_after(match) and not matches.range(
                    match.end, match.end + 1, lambda m: "source-suffix" in m.tags
                ):
                    if match.children:
                        ret.extend(match.children)
                    ret.append(match)
    ...
    def when(self, matches: Matches, context: dict[str, Any] | None) -> Any:
        ...
        for filepart in matches.markers.named("path"):
            for match in matches.range(filepart.start, filepart.end, predicate=lambda m: m.name == "source"):
                ...
                if (
                    "weak.source" in match.tags
                    and matches.range(match.end, filepart.end, predicate=lambda m: m.name == "source")
                    and matches.holes(filepart.start, match.start, predicate=lambda m: m.value.strip(seps), index=-1)
                ):
                    if match.children:
                        ret.extend(match.children)
                    ret.append(match)
