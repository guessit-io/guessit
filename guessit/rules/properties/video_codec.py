...
def video_codec(config: dict[str, Any]) -> Rebulk:
    ...
    rebulk.regex(
        "(?P<video_codec>hevc)(?P<color_depth>10)",
    ...
    rebulk.string("DXVA", value="DXVA", name="video_api", disabled=lambda context: is_disabled(context, "video_api"))
    ...
    def when(self, matches: Matches, context: dict[str, Any] | None) -> Any:
        ...
        for codec in matches.named("video_codec"):
            if not seps_before(codec) and not matches.at_index(
                ...
                ret.append(codec)
                continue
            if not seps_after(codec) and not matches.at_index(
                codec.end + 1, lambda match: "video-codec-suffix" in match.tags
            ):
                ret.append(codec)
                continue
        return ret
